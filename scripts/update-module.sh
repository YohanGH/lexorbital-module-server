...
# Script: update-module.sh
# Purpose: Update an existing module using git subtree
# Usage: ./scripts/update-module.sh <module-name> <repo-url> [branch]
# Example: ./scripts/update-module.sh lexorbital-module-auth https://github.com/lexorbital/lexorbital-module-auth.git main
...
check_module_exists() {
  ...
  if [ ! -d "$target_dir" ]; then
    ...
    echo "   Use add-module.sh to add a new module"
    exit 1
  fi
}
...
if [ -z "$MODULE_NAME" ] || [ -z "$MODULE_REPO" ]; then
  ...
  echo "Example: ./scripts/update-module.sh lexorbital-module-auth https://github.com/lexorbital/lexorbital-module-auth.git main"
  exit 1
fi
...
info "Updating module ${MODULE_NAME}..."
...
# Pull the changes
if ! git subtree pull --prefix="$TARGET_DIR" "${REMOTE_NAME}" "$MODULE_BRANCH" --squash; then
  error "Failed to pull subtree changes"
  cleanup_remote "$REMOTE_NAME"
  exit 1
fi
...
success "Module ${MODULE_NAME} successfully updated"
